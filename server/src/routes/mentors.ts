...
router.post('/', authMiddleware, (req: Request, res: Response) => {
  const { title, name, avatar, research, college, email, office, academic_title, sort_order } = req.body;
  if (!name) { res.status(400).json({ error: 'name 为必填字段' }); return; }
  const stmt = db.prepare('INSERT INTO mentors (title, name, avatar, research, college, email, office, academic_title, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
  const result = stmt.run(title || '', name, avatar || '', research || '', college || '', email || '', office || '', academic_title || '', sort_order || 0);
...
router.put('/:id', authMiddleware, (req: Request, res: Response) => {
  ...
  const stmt = db.prepare('UPDATE mentors SET title=?, name=?, avatar=?, research=?, college=?, email=?, office=?, academic_title=?, sort_order=? WHERE id=?');
  stmt.run(
